...
<div>
        ...
        <modal :show.sync="showModal"
            ...
            <div slot="body" class="border-top border-bottom">
                ...
                <div class="col col-6 border-left border-right">
                    <div v-html="sharedState.state.selectedContent.markdown | marked | referenced" class="p1" style="height: 700px; overflow-y: scroll"></div>
                </div>
    ...
    <input v-model="searchContent" class="col-right" type="text" name="search" placeholder="&#128269;  Search&hellip;" style="border: none">
    ...
    <div v-for="content in content | filterBy searchContent"
        ...
        <div class="col col-12">
            ...
            <div class="col col-right">
                ...
                <!--Create/Edit buttons-->
                <button v-show="!content.id" @click.prevent="createContent($index)" class="btn btn-outline green px0 small unbold">Create</button>
                <button v-show="content.id"  @click.prevent="openModal()"           class="btn btn-outline green px0 small unbold">Edit</button>

                <!--Preview-->
                <a href="/content/{{ content.slug }}" v-show="content.id" class="btn border rounded small unbold mr1" target="_blank">preview</a>
                <!--Toggle active-->
                <a href="#" v-show="content.id" @click.prevent="toggleActive(content)" class="btn border rounded small unbold {{ content.active ? 'green' : 'red' }}">{{ content.active ? 'active' : 'inactive' }}</a>
                <!--Delete-->
            ...
            <div v-show="!content.id" class="col col-right">
                <button @click.prevent="removeContent(content)" class="col col-right mt1 block btn border rounded">&minus;</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        events: {
            ...
            'add-reference-to-content'(reference) {
                ...
                if( ! _.where(selectedContentReferences, { no: reference.no }).length)
                {
                    ...
                    self.$dispatch('messenger-notify', { content: `Reference numbered <strong>${reference.no}</strong> was added to this content!`, type: 'success' });
                } else {
                    self.$dispatch('messenger-notify', { content: `There is already a reference numbered <strong>${reference.no}</strong> in the list `, type: 'info' });
                }
        ...
        methods:  {
            ...
            createContent(index){
                var self = this;
                Ajax.put(this.routes.createContent, JSON.stringify(self.content[index])).then(function (response) {
                    var data = response.data;
                    self.content.splice(index, 1);
                    self.content.push(data);
                    self.$dispatch('fetch');
                    self.$dispatch('messenger-notify', {
                        content: "Added content: " + (data.name) +
                                "<div class='mt1'>" +
                                    " <a href='#' class='muted'>preview content</a>" +
                                "</div>" +
                                "<div>" +
                                    " <a href='#pages' class='muted'>add this content to a page</a>" +
                                "</div>",
                        type: 'success'
                    });
                }, function (response) {
            ...
            updateContent() {
                var self = this;
                Ajax.patch(`${this.routes.updateContent}/${this.sharedState.state.selectedContent.id}`, JSON.stringify(this.sharedState.state.selectedContent)).then(function (response) {
                    var data = response.data;
                    self.$dispatch('fetch');
                    self.$dispatch('messenger-notify', { content: "Updated content"+"<div>"+(data.name)+ " <a href='#' class='muted'> preview page</a>" + "</div>", type: 'success' });
                }, function () {
            ...
            deleteContent(content) {
                var self = this;
                Ajax.delete(this.routes.deleteContent + '/' + content.id).then(function (response) {
                    var data = response.data;
                    self.$dispatch('fetch');
            ...
            toggleActive(content) {
                var self = this;
                Ajax.get(this.routes.toggleContentActive + '/' + content.id).then(function (response) {
                    var data = response.data;
                    self.$dispatch('fetch');
